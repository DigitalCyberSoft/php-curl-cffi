...
<?php
  ...
  curl_cffi_setopt ( $ch , CURLOPT_FTP_USE_EPSV, 0 );
  curl_cffi_setopt ( $ch , CURLOPT_FTP_SKIP_PASV_IP, 1 );

  $cmh =  curl_cffi_multi_init();
  curl_cffi_multi_add_handle($cmh, $ch);

  $active = null;

  do {
      $mrc = curl_cffi_multi_exec($cmh, $active);
  } while ($mrc == CURLM_CALL_MULTI_PERFORM);


  while ($active && $mrc == CURLM_OK) {
      if (curl_cffi_multi_select($cmh) != -1) {
          do {
              $mrc = curl_cffi_multi_exec($cmh, $active);
          } while ($mrc == CURLM_CALL_MULTI_PERFORM);
      }
  ...
  var_dump(is_string(curl_cffi_multi_getcontent($ch)));
  curl_cffi_multi_remove_handle($cmh, $ch);
  curl_cffi_close($ch);
