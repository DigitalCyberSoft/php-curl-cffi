--TEST--
Test curl_cffi_copy_handle() add options to the handles
--CREDITS--
...
curl_impersonate
--DESCRIPTION--
the only way to test if a option is setten on a curl handle is using the curl_cffi_getinfo() function.
but this can only check on a limited amount of options...
--FILE--
<?php
echo "*** Testing curl_cffi_copy_handle(): add options after copy ***\n";
...
curl_cffi_setopt($ch2, CURLOPT_URL, 'http://www.example.com/');
...
?>
--EXPECT--
*** Testing curl_cffi_copy_handle(): add options after copy ***
bool(true)
bool(false)
bool(true)
bool(false)
