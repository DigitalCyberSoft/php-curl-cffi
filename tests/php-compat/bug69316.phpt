...
<?php
  function hdr_callback($ch, $data) {
      ...
      if($GLOBALS['f_file']) {
          ...
          curl_cffi_setopt($ch, CURLOPT_COOKIE, str_repeat("a", $FILE_size - 1));
      }
      return strlen($data);
  }
  ...
  curl_cffi_setopt($ch, CURLOPT_BUFFERSIZE, 10);
  curl_cffi_setopt($ch, CURLOPT_HEADERFUNCTION, "hdr_callback");
  curl_cffi_setopt($ch, CURLOPT_FILE, $f_file);
...
?>
--EXPECTF--
Warning: curl_cffi_exec(): CURLOPT_FILE resource has gone away, resetting to default in %s on line %d
array(1) {
  ["test"]=>
  string(7) "getpost"
}
