--TEST--
curl_cffi_multi_add_handle does not hold onto the handle on failure
--EXTENSIONS--
curl_impersonate
--FILE--
<?php
...
foreach ($urls as $url) {
    ...
    curl_cffi_setopt($ch, CURLOPT_PRIVATE, new MyClass());

    if (curl_cffi_multi_add_handle($mh, $ch) == CURLM_OK) {
        $toRemove[] = $ch;
    }
    if (curl_cffi_multi_add_handle($mh, $ch) == CURLM_OK) {
        $toRemove[] = $ch;
