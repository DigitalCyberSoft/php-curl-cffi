...
curl_cffi_setopt($ch, CURLOPT_VERBOSE, 0);
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, function() {
...
curl_cffi_setopt($ch, CURLOPT_STDERR, $stderr);
$curlVerboseOutput = curl_cffi_exec($ch);
rewind($stderr);
...
$debugOutput = [];
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, function(CurlImpersonate\Curl $curlHandle, int $type, string $data) use ($allowedTypes, &$debugOutput) {
...
$ch = curl_cffi_init();
var_dump(curl_cffi_setopt($ch, CURLINFO_HEADER_OUT, true));
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, null));
var_dump(curl_cffi_setopt($ch, CURLINFO_HEADER_OUT, true));
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, 'strlen'));
try {
    var_dump(curl_cffi_setopt($ch, CURLINFO_HEADER_OUT, true));
}
catch (\ValueError $e) {
...
$chCopy = curl_cffi_copy_handle($ch);
try {
    var_dump(curl_cffi_setopt($chCopy, CURLINFO_HEADER_OUT, true));
}
catch (\ValueError $e) {
    var_dump($e->getMessage());
}
var_dump(curl_cffi_setopt($chCopy, CURLOPT_DEBUGFUNCTION, null));
var_dump(curl_cffi_setopt($chCopy, CURLINFO_HEADER_OUT, true));
...
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, 'strlen'));
var_dump(curl_cffi_setopt($chCopy, CURLOPT_DEBUGFUNCTION, null));
var_dump(curl_cffi_setopt($chCopy, CURLINFO_HEADER_OUT, 1));

echo "\n===Test CURLOPT_DEBUGFUNCTION=null works===\n";
$ch = curl_cffi_init("{$host}/get.inc?test=file");
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, null));
curl_cffi_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
$ch = curl_cffi_init("{$host}/get.inc?test=file");
curl_cffi_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
var_dump(curl_cffi_getinfo($ch, CURLINFO_HEADER_OUT));
var_dump(curl_cffi_setopt($ch, CURLINFO_HEADER_OUT, true));
var_dump(curl_cffi_getinfo($ch, CURLINFO_HEADER_OUT));
var_dump(curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, static function() {}));
var_dump(curl_cffi_getinfo($ch, CURLINFO_HEADER_OUT));
var_dump($result = curl_cffi_exec($ch));
var_dump(curl_cffi_getinfo($ch, CURLINFO_HEADER_OUT));

echo "\n===Test CURLOPT_DEBUGFUNCTION can throw within callback===\n";
$ch = curl_cffi_init("{$host}/get.inc?test=file");
curl_cffi_setopt($ch, CURLOPT_DEBUGFUNCTION, static function() {
...
curl_cffi_exec($ch2);
var_dump($called);
var_dump(curl_cffi_getinfo($ch2, CURLINFO_HEADER_OUT));
